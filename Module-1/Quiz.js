...
1. Choose all applicable properties of a Hash
 ...
 A hash is deterministic, in that, calculating it twice yields the same result /CORRRECT/
 A small change in the original value yields a smaller change in the hash than a big change in the original value
 The probability of finding 2 different values that compute to the same hash is extremely low /CORRECT/

 2. What can we use a Merkle Tree for?
 
 Losslessly compress data by calculating the merkle root
 Compare complex collections of data by comparing the merkle root /CORRECT/
 ...
 Traverse the leaves of the tree depth-first /CORRECT/
 ...
 a store indexed by hash intrinsically protects me from error and malice, unlike one by arbitrary id /CORRECT ???/
 a store indexed by hash can grow much larger than one by arbitrary id
 a store indexed by arbitrary id can hold duplicate values, unlike one by hash /CORRECT????/
 
 5. What is the double-spend problem within Bitcoin?
 
 someone sends 2 different transactions at the same time to the network, in order to send the same coin to 2 different recipients before the nodes can spot the fraud /CORRECT/
 someone sends the coin to a recipient in exchange for a consideration, then waits to receive the consideration, then it just so happens that the consensus switches to a fork that does not contain the previous transaction
 someone sends the coin to a recipient in exchange for a consideration, then waits to receive the consideration, then builds a fork that does not contain the previous transaction and that is acceptable to the network
